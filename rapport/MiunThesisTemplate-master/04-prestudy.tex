...
LLM:en tolkar användarens fråga och avgör själv vilket eller vilka verktyg som ska anropas för att hämta information från rätt system.
...
\subsubsection{Vektordatabas / embeddings (RAG)}
...
Denna lösning varken implementeras eller undersöks vidare i detta arbete då den liksom den API-baserade lösningen inte förändrar hur relationer och samband mellan data från olika källor representeras.
...
Grafdatabasens syfte i detta arbete är att lagra data och information från de olika interna systemen i form av en kunskapsgraf.
...
Databasen som väljs är Neo4j. Detta val görs då det är en marknadsledande grafdatabas med stor användarbas och bra community-stöd.
Den har en låg inlärningströskel med omfattande dokumentation och många guider för att komma igång. Den är även av typen Online Analytical Processing (OLAP), vilket betyder att den är specifikt anpassad för just affärsintelligens och informationssökning vilket är det primära syftet för databasen i detta arbete\cite{17}.
...
\subsection{Design av kunskapsgrafen och grafdatabasen}\label{subsec:dkg}
...
\textit{Project}, \textit{Repository}, \textit{Issue}, \textit{Commit}, \textit{Push}, \textit{Service},
\textit{SlackChannel}, \textit{SlackThread}, \textit{Post}, \textit{ConfluenceSpace} och
...
Figur \ref{fig:yyy} visar en bild över alla relationer till användaren \textit{Isak}.

\begin{figure}[H]
  \includegraphics[width=15cm]{Figures/kg_quests/tester9PIC.png}
  \caption{Relationer till användaren \textit{Isak}}\label{fig:yyy}
\end{figure}

Eftersom att det inte går att avläsa etiketterna och relationerna i ovanstående bild, visar istället Figur \ref{fig:labels} etiketterna till entiteterna med färgkodningen som används.

\begin{figure}[H]
  \includegraphics[width=11cm]{Figures/labels.png}
...
De medelsvåra frågorna (Q6–Q10) kräver antingen flera anrop till samma datakälla eller lite mer avancerat resonemang för att nå svaret.
...
Samtliga frågor presenteras i kapitel \ref{subsec:questTest}.

\subsection{Utvärderingsstrategi}
